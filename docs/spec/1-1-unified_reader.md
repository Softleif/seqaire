...
`Readers::pileup(tid, start, end)` MUST: (1) resolve the tid argument via `ResolveTid`; (2) call `fetch_into_filtered` to load records into the internal `RecordStore<()>`, passing the provider's `keep_record` as the filter — records rejected by the filter are rolled back with zero slab waste (BAM/SAM) or dropped via a post-fetch retain pass (CRAM); (3) fetch the reference sequence for `[start, end]` inclusive via the FASTA reader; (4) rebuild the store as `RecordStore<E::Extra>` by running the provider over every kept record via `apply_extras`; (5) construct a `PileupEngine<E::Extra>` with the typed store and the fetched reference sequence pre-attached via `set_reference_seq`. The returned engine is ready for `pileups()` iteration with no further configuration required.

r[unified.fetch_into_filtered]
Each format reader (`IndexedBamReader`, `IndexedSamReader`, `IndexedCramReader`, and the format-agnostic `IndexedReader`) MUST expose `fetch_into_filtered(tid, start, end, store, keep) -> Result<FetchCounts>` in addition to `fetch_into`. `FetchCounts { fetched, kept }` reports records produced by the reader's built-in overlap/unmapped checks (`fetched`) vs those that also passed the user's `keep` predicate (`kept`). Existing `fetch_into` MUST remain a thin wrapper passing an always-true filter so its signature and behavior are unchanged.
