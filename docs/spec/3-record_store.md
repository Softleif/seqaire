...
`RecordStore<()>` MUST provide `apply_extras<E: RecordStoreExtras>(self, provider: &mut E) -> RecordStore<E::Extra>` that computes one `E::Extra` per record by calling `provider.compute(idx, &self)`. The provider is reusable: Readers holds one provider and applies it on every region fetch, and may carry state (counters, caches) that persist across regions. The trait is defined as `trait RecordStoreExtras: Clone { type Extra; fn compute(&mut self, idx: u32, store: &RecordStore<()>) -> Self::Extra; fn keep_record(&mut self, rec: &SlimRecord, store: &RecordStore<()>) -> bool { true } }` — the `Clone` bound allows `Readers::fork` to duplicate the provider into the forked reader. A blanket `impl RecordStoreExtras for ()` MUST exist with `type Extra = ()` so the default no-extras case costs zero at runtime (`Vec<()>` is a ZST vector). `RecordStore` MUST NOT expose a closure-based `with_extras(f)` method — the trait is the only API, so providers are always reusable and clone-forwardable.

r[record_store.pre_filter.rollback]
`RecordStore<()>::push_raw` and `RecordStore<()>::push_fields` MUST accept a `keep: FnMut(&SlimRecord, &RecordStore<()>) -> bool` closure as their last argument and return `Result<Option<u32>, DecodeError>`. When `keep` returns `false`, the record just pushed MUST be rolled back so that every slab (names/bases/cigar/qual/aux/records/extras) is byte-identical to its pre-push state. Rollback uses the just-pushed `SlimRecord`'s `*_off` offsets as pre-push slab lengths, since push is append-only. The returned `Option<u32>` is `Some(idx)` when kept, `None` when rejected — rejected records MUST NOT consume an index. A proptest verifies that pushing a mixed accept/reject sequence produces byte-identical state to pushing only the accepted inputs with no filter.

r[record_store.pre_filter.provider_hook]
`RecordStoreExtras` MUST provide a default-`true` method `fn keep_record(&mut self, rec: &SlimRecord, store: &RecordStore<()>) -> bool` that `Readers::pileup` passes through to `push_raw`/`push_fields` via `fetch_into_filtered`. This lets users co-locate extras computation and per-record filtering in one provider type. The filter fires *before* `apply_extras`, so `compute` is only ever called for kept records.

r[record_store.pre_filter.retain]
`RecordStore<U>` MUST provide `retain<F: FnMut(&SlimRecord) -> bool>(&mut self, predicate: F)` that removes records where the predicate returns `false`. Unlike the push-time filter, `retain` runs post-push: slab data for dropped records is left in place as dead bytes until `clear()`. CRAM's `fetch_into_filtered` uses `retain` because plumbing a filter through the CRAM decode path would be invasive; BAM/SAM filter at push time with zero waste.
