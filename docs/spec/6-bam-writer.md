...
`BamWriter::build_over(writer, header).build()` MUST write BGZF-compressed BAM to a caller-supplied [`Write`] sink (e.g. `io::stdout()`). `write_index(true)` is accepted on this path for API uniformity, but it is a soft no-op: there is no sidecar `.bai` location and BAI virtual offsets are only meaningful against a seekable file, so no `IndexBuilder` is constructed and an `info!` log records the dropped request. The caller is responsible for determining whether stdout is the intended target (e.g. by checking for `-` or `/dev/stdout` in a CLI argument).
